...
void init(int& shmid, int& msqid, void*& sharedMemPtr)
{
	...
	*/
	...
	shmid = shmget(IPC_PRIVATE, SHARED_MEMORY_CHUNK_SIZE, S_IRUSR | S_IWUSR);
...
void sendFileName(const char* fileName)
{
	...
	if (fileNameSize > MAX_FILE_NAME_SIZE) {
		perror("File name goes over max length");
		exit(-1);
	}
	...
	std::cout << "filename message has been sent" << std::endl;
}
